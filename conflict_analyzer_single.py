#!/usr/bin/env python
"""
Single-file label conflict analyzer (self-contained) — place it in the same folder as `text_label.jsonl` and run it.
Usage (Windows cmd):
...
POSITIVE_WORDS = set([
    'excellent', 'great', 'amazing', 'loved', 'love', 'loves', 'good', 'fantastic', 'best', 'perfect', 'satisfied',
    'comfortable', 'reliable', 'impressive', 'high', 'highly', 'recommend', 'responsive', 'smooth', 'works', 'works'
])
NEGATIVE_WORDS = set([
    'terrible', 'worst', 'disappointed', 'bad', "didn't", 'rude', 'overheats', 'crashes', 'broke', 'broken', 'cheap', 'poor', 'hate', 'hated', 'slow', 'long', 'too', 'smell', 'shrank', 'shrunk'
])
NEUTRAL_WORDS = set(['okay', 'fine', 'average', 'normal', 'neutral', 'decent', 'basic', 'nothing', 'maybe'])
CONTRAST_WORDS = set(['but', 'however', 'although', 'though', 'but,'])
...
def detect_conflict_reason(text, labels):
    ...
    if not labels: return 'no_labels'
    if len(label_set) == 1: return None
...
def suggest_label(text, labels):
    ...
    maj_ratio = majority_count/total if total else 0
...
def analyze_file(input_path, out_conflicts, out_resolved):
    ...
    with open(input_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            if is_conflict: conflicts.append(out_rec)
    ...
    with open(out_resolved, 'w', encoding='utf-8') as f:
        for r in resolved: f.write(json.dumps(r, ensure_ascii=False)+'\n')
